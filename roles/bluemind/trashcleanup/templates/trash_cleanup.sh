...
while read junkf;do
    ...
    mbox=$(echo $junkf |sed -re "s/.*\/(user\/.*)$/\1/" -e "s/\//./g" -e "s/ /\%20/g")
    log "Reconstructing $mbox@$domain"
    log su cyrus -c "reconstruct -U -f $mbox@$domain"
done < <(find "${CYRUS_SPOOL}" \( -iname trash -or -iname "deleted messages" \) -and -type d)
